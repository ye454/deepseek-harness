...
import WorkConsoleCommandService from '../src/index.ts'
...
async function harness() {
  ...
  await ctx.plugin(WorkConsoleCommandService)
  return ctx
}
...
describe('WorkConsoleCommandService promoteIdea', () => {
  it('promotes explicitly to organizing without doing organization work', async () => {
    ...
    const result = await ctx.workConsoleCommands.promoteIdea({ id: String(idea.id), revision: idea.revision, priority: 'p0' })
  ...
  it('uses P2 by default and rejects missing, stale, and already-promoted work', async () => {
    ...
    expect(await ctx.workConsoleCommands.promoteIdea({ id: 'missing', revision: 1 })).toMatchObject({
      ok: false, error: { code: 'not-found' },
    })
    expect(await ctx.workConsoleCommands.promoteIdea({ id: String(idea.id), revision: idea.revision + 1 })).toMatchObject({
      ok: false, error: { code: 'conflict', currentRevision: idea.revision },
    })
    const first = await ctx.workConsoleCommands.promoteIdea({ id: String(idea.id), revision: idea.revision })
    expect(first).toMatchObject({ ok: true, value: { priority: 'p2' } })
    const task = currentTask(ctx, idea.id)
    expect(await ctx.workConsoleCommands.promoteIdea({ id: String(task.id), revision: task.revision })).toMatchObject({
  ...
  it('maps promotion races and rethrows infrastructure failures', async () => {
    ...
    expect(await ctx.workConsoleCommands.promoteIdea({ id: String(conflictIdea.id), revision: conflictIdea.revision })).toMatchObject({
    ...
    expect(await ctx.workConsoleCommands.promoteIdea({ id: String(stateIdea.id), revision: stateIdea.revision })).toMatchObject({
    ...
    await expect(ctx.workConsoleCommands.promoteIdea({ id: String(brokenIdea.id), revision: brokenIdea.revision }))
      .rejects.toThrow('storage down')
    await ctx.fiber.dispose()
  })
})

describe('WorkConsoleCommandService decideAcceptance', () => {
  it('rejects missing/idea/stale-revision/non-validation requests before mutation', async () => {
    const ctx = await harness()
    expect(await ctx.workConsoleCommands.decideAcceptance({
  ...
  it('accepts with a Host-owned harness-home actor and completes when every required gate passed', async () => {
    ...
    const result = await ctx.workConsoleCommands.decideAcceptance({
  ...
  it('keeps validation open when another required human gate still remains', async () => {
    ...
    const first = await ctx.workConsoleCommands.decideAcceptance({
      taskId: String(task.id), taskRevision: current.revision, generation: session.generation, validatorIndex: 1, decision: 'accept',
    })
    expect(first).toMatchObject({ ok: true, value: { status: 'validation' } })

    current = currentTask(ctx, task.id)
    const second = await ctx.workConsoleCommands.decideAcceptance({
  ...
  it('maps validation races and propagates unexpected infrastructure failures', async () => {
    ...
    await expect(ctx.workConsoleCommands.decideAcceptance({
      taskId: String(ready.task.id), taskRevision: ready.task.revision, generation: ready.session.generation, validatorIndex: 1, decision: 'accept',
  ...
  it('maps Work Control conflicts after accepted/rejected validation decisions', async () => {
    ...
    const acceptSetStatus = vi.spyOn(acceptCtx.workControl, 'setStatus')
    acceptSetStatus.mockRejectedValueOnce(new WorkItemConflictError(
      { id: accepted.task.id, revision: accepted.task.revision + 1 },
      accepted.task.revision + 2,
    ))
    const acceptResult = await acceptCtx.workConsoleCommands.decideAcceptance({
    ...
    const returnSetStatus = vi.spyOn(returnCtx.workControl, 'setStatus')
    returnSetStatus.mockRejectedValueOnce(new WorkItemConflictError(
      { id: returned.task.id, revision: returned.task.revision + 1 },
      returned.task.revision + 2,
    ))
    const returnResult = await returnCtx.workConsoleCommands.decideAcceptance({
